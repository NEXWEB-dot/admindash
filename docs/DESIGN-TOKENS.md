...
Extracted from `index.html` in the **groundsnearme-frontend** repository so the owner
dashboard, the admin view and the private finance dashboard can match the public site
exactly rather than approximately. The
...
The public site is now a separate repository, so this file cannot be checked against
it automatically. When the frontend devs change a token, this document and
`public/css/tokens.css` both need the same edit — the two surfaces drift apart one
